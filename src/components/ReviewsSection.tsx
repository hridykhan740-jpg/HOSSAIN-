    ...
    <section>
      <div className="flex flex-col md:flex-row items-center justify-between mb-16 gap-6">
        ...
        <motion.button
          ...
          className="flex items-center gap-2 bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600 hover:text-slate-900 dark:text-white px-6 py-3 rounded-full font-medium transition-colors border border-emerald-500/30"
        >
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-slate-900 border border-white/10 rounded-3xl p-8 max-w-lg w-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input required value={name} onChange={e=>setName(e.target.value)} className="w-full bg-slate-100 dark:bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500" />
              </div>
              <div>
